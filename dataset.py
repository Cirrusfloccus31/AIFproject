...
class Movie_Dataset(Dataset):
    ...
    def __init__(self, split, data_path = DATA_PATH, training_fraction = 0.7):
        ...
        self.genres = [genres for genres in os.listdir(data_path) if os.path.isdir(os.path.join(data_path, genres))]
        self.num_classes = len(self.genres)
        self.paths = list(Path(data_path).glob("**/*.jpg")) #List all the paths of the images
    ...
    def __getitem__(self, idx):
        ...
        y = torch.nn.functional.one_hot(torch.tensor(self.genres.index(genre_str)), num_classes=self.num_classes).to(torch.float)
        return x, y
