...
def test(net, test_loader):
    ...
    for x, y in test_loader:
        ...
        with torch.no_grad():
            outputs = net(x)
        confusion_matrix[
            torch.argmax(outputs, dim=1).item(), torch.argmax(y, dim=1).item()
        ] += 1
    ...
    fig = plt.figure()
    plt.imshow(
        confusion_matrix, origin="upper", interpolation="nearest", aspect="equal"
    )
    ax = plt.gca()
    ...
    plt.title(
        f"Confusion Matrix on the test dataset Global accuracy = {round(accuracy, 3)}"
    )
    plt.tight_layout()
